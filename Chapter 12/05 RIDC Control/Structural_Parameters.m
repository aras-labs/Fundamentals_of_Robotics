...
SP.pert = 0.5;              % (1-SP.pert)*100% parameter perterbation
...
SP.Kp=1e4*diag([1 2 0.2]); % The proportional controller gain Kp
SP.Kd=1e4*diag([1 2 0.2]); % The  derivative  controller gain Kd
SP.Ki=1e5*diag([1 2 0.2]); % The  integral controller gain Ki

%%
%   Robust Control Term
%
A=[zeros(SP.n)  eye(SP.n)
    -SP.Kp      -SP.Kd];
Q=diag([5, 2, 0.1, 5, 2, 0.1]);
SP.B=[zeros(SP.n)
        eye(SP.n)];
SP.P=lyap(A',Q);
SP.rho=1e5;
