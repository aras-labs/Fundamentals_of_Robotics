%% 
% 	Fundamentals of Robotics: Practical Case Studies with Matlab & Python
%   By Prof. Hamid D. Taghirad
%   Copyright CRC - Press @2025
%
%   This program generates all the requires parameters of the 
%   programs in a structure.
%
function [SP] = Structural_Parameters()
%% 
deg2rad = pi/180 ; rad2deg = 180/pi;

%% Kinematics and Dynamics Parameters

SP.n   = 3;                 % No. of degrees of Freedom
SP.a   = [1 ; 0.7; 0.3];    %   Link lengths
SP.m = [10; 7; 3];          % the Mass of the E.F. (Kg)
SP.g = -9.81;               % Gravity Acceleration (m/s^2)
SP.pert = 0.2;                % (1-SP.pert)*100% parameter perterbation

%% Stiff Environment
%  A linear surface passes throug a point SP.xp with a slope Sp.sl

SP.xl = 1.1; SP.yl = 1.1;         

SP.sl=1;                    % The slope of the line

SP.Ke = 1e4*diag([1 1 0]);   % Environment stiffness coefficient
SP.Ce = 0e3*diag([1 1 0]);   % Environment damping coefficient

%% Desired Trajectory in task space

SP.phi = -atan(SP.sl);   % Normal to the flat object surface
x0 = SP.xl; y0= SP.yl;
xf = 0.5; yf= SP.yl + SP.sl*(xf-SP.xl); % Find the second point coordinates
SP.del=0.1; del=SP.del;

%   The desired trajectory initial points
%     Time   x          y       \phi
SP.xd=[
        0    x0+del   y0-del     SP.phi
        3    xf+del   yf-del     SP.phi
        4    xf+del   yf-del     SP.phi
        6    0          0         0
        8    0          0         0
        10   0          0         0
        ];
SP.ts = 0 ; SP.tf = 4;
    
%% load Noise file
load snoise.mat;
SP.noise=qnoise;
SP.Isn=0;       % 0: no noise, 1: Is noise
clear qnoise

%% External Disturbance
%   The desired trajectory initial points
%     Time    \tau_1   \tau_2   \tau_3
taud=[
     0       0       0       0
    1        0       0       0
    1.001   -100   -100    -10
    10      -100   -100    -10
  ];
SP.time=taud(:,1);   
SP.taud=taud(:,2:4);    
SP.Isd=1;       % 0:no disturbance;   1:Is disturbance
%%
%  Actuator limitations
%
SP.tau_limit=[150; 60; 5];    % Torque limits
SP.Istau=0;      % 0:no saturation;   1:With Saturation

%% Simulation Parameters
SP.tspan=[SP.ts SP.tf];                       
SP.x0 =  [x0-del/10; y0+del/10; SP.phi];
SP.qd0 = [0 ; 0 ; 0];
SP.qi0 = [0 ; 0 ; 0];  % initial condition for qi
SP.xi0 = [0 ; 0 ; 0];  % initial condition for xi

%% Controller Parameters
    
SP.Kp=1e3*diag([1 2 0.2]); % The proportional controller gain Kp
SP.Kd=1e3*diag([1 2 0.2]); % The  derivative  controller gain Kd
SP.Ki=0e3*diag([1 2 0.2]); % The  integral controller gain Ki

% Normalized Gains with M
% SP.Kp=5*diag([1 1 1]); % The proportional controller gain Kp
% SP.Kd=5*diag([1 1 1]); % The  derivative  controller gain Kd
% SP.Ki=0.0*diag([1 1 1]); % The  integral controller gain Ki

end